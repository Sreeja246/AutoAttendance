...
namespace AutoAttendance.Controllers
{
    public class StudentController : Controller
    {
        ...
        [HttpPost]
        public ActionResult Attendance(Student students, string[] Present, string submitButton, string[] late)
        {
            ...
            foreach (var student in studentList)
            {
                ...
                if (pos > -1)
                {
                    stud.Present = "P";
                    int posLate = -2;
                    var lateId = "late_" + student.Id;
                    if (late != null)
                    {
                        posLate = Array.IndexOf(late, lateId);
                        if (posLate > -1)
                        {
                            stud.Present = "L";
                        }
